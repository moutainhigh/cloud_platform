...
JAVA_OPTS="-Duser.timezone=GMT+8 -server -Xms124m -Xmx1024m  -XX:+PrintGCTimeStamps -XX:+PrintGCDetails -Xloggc:./gc.log -XX:+HeapDumpOnOutOfMemoryError"
...
#        nohup $JAVA_HOME/bin/java $JAVA_OPTS -classpath $CLASSPATH $APP_MAIN >/dev/null 2>&1 &
        $JAVA_HOME/bin/java $JAVA_OPTS $DEBUG -classpath $CLASSPATH $APP_MAIN
        getPID
        if [ $PID -ne 0 ]; then
