...
import cn.sensordb2.stcloud.util.CreateFileUtil;
...
public class HYHttpServerLogger {
    ...
    public HYHttpServerLogger(Logger logger) {
        this.logger = logger;
    }

    public static HYHttpServerLogger getLogger(Class cls) {
        return new HYHttpServerLogger(Logger.getLogger(cls));
    }
    ...
    public void info(Object msg) {
        ...
        if(HYHttpServerLogger.hasTrackedUser()) return ;
    ...
    public void info(Object msg, NetSocket socket) {
        try {
            HYHttpServerLogger.changeAllLoggerFile(socket);
            ...
            if(ci!=null&&ci.getUserID()!=null) {
                ...
                if(!HYHttpServerLogger.isTrackedUser(userID)) return;
    ...
    public void error(Object msg, NetSocket socket) {
        try {
            if (IniUtil.getInstance().isLogErrorToDatabase()) {
                ...
                if(mongoClient!=null) {
                    mongoClient.insert(HYHttpServerLogger.ERROR_LOG_TABLE, errorJson, result -> {
                    });
                }
            }
            HYHttpServerLogger.changeAllLoggerFile(socket);
    ...
    public void info(Object msg, ConnectionInfo ci) {
        try {
            HYHttpServerLogger.changeAllLoggerFile(ci);
            if(ci!=null&&ci.getUserID()!=null) {
                String userID = ci.getUserID();
                if(!HYHttpServerLogger.isTrackedUser(userID)) return;
    ...
    public void receivedDataLogJson(Buffer buffer, NetSocket socket, boolean isTemp) {
        ...
        ConnectionInfo ci = ClientManager.getInstance().getConnectionInfo(socket.hashCode());
        if(ci!=null&&ci.getUserID()!=null) {
            if(!HYHttpServerLogger.isTrackedUser(ci.getUserID())) return;
            ...
            if(!HYHttpServerLogger.isTrackedUser(ci.getPhoneNumber())) return;
    ...
    public static void changeAllLoggerFile(NetSocket socket) {
        try {
            ...
            HYHttpServerLogger.changeAllLoggerFile(path, fileName);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void changeAllLoggerFile(String path, String filename) {
        try {
            if(!IniUtil.getInstance().isAllInOneLogFile()) {
                HYHttpServerLogger.changeDebugLoggerFile(path, filename);
                HYHttpServerLogger.changeErrorLoggerFile(path, filename);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
 ...
 */
    ...
    */
    ...
    public static void resetAllLoggerFile() {
        HYHttpServerLogger.resetDebugLoggerFile();
        HYHttpServerLogger.resetErrorLoggerFile();
    }
